...
/++
    The main Box3D API: world, body, shape, joint, contact and
    recording/replay functions.

    Translated from `box3d/box3d.h`.
+/
module box3d.functions;
...
// ---------------------------------------------------------------------------
// Recording / replay types (opaque handles and metadata)
// ---------------------------------------------------------------------------
...
struct b3RecPlayerInfo
{
    ...
    float timeStep = 0; /// dt of the recorded steps
    int subStepCount; /// recorded sub-steps
    float lengthScale = 0; /// length units per meter in effect when recorded
...
struct b3RecQueryInfo
{
    ...
    ulong key; /// identity key, the hash of (id, name), 0 if untagged
    ulong id; /// query id, 0 if none
...
// Functions
//
// In the default single-precision build `b3CreateWorld` is a real exported
// symbol. (In BOX3D_DOUBLE_PRECISION builds the C header remaps it to
// b3CreateWorldDoublePrecision; this binding targets single precision.)
// ---------------------------------------------------------------------------
...
void b3World_Draw(b3WorldId worldId, b3DebugDraw* draw, ulong maskBits);
...
b3TreeStats b3World_OverlapShape(b3WorldId worldId, b3Pos origin, const(b3ShapeProxy)* proxy,
...
float b3World_CastMover(b3WorldId worldId, b3Pos origin, const(b3Capsule)* mover, b3Vec3 translation,
...
const(ubyte)* b3Recording_GetData(const(b3Recording)* recording);
...
int b3Body_CollideMover(b3BodyId bodyId, b3BodyPlaneResult* bodyPlanes, int planeCapacity, b3Pos origin,
